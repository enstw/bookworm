// Matcha acoustic model + Vocos vocoder under onnxruntime-web. Ported from the
// sibling research repo ~/workspace/wasmtts (platform/matcha-synthesis.js @
// 05b5b35) and kept byte-for-byte below this header, so upstream stays diffable.
//
// Vocos does not return a waveform: it emits magnitude and cos/sin phase
// (mag/x/y), so the inverse FFT and overlap-add below are required to get audio
// at all, not an optimisation someone added. They cost ~1.4% of synthesis time,
// which is why they are plain JS. scaleSilence is a port of sherpa-onnx's
// GeneratedAudio::ScaleSilence (Apache-2.0) and is what keeps the model's long
// inter-sentence silences from dragging.
//
// IIFE global with a CommonJS tail — see the note in matcha-frontend.js.
