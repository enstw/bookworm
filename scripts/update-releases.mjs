...
// `released` tag, newest first; the workflow step then commits the file and
// moves the tag onto that docs commit — so the next range starts after it and
// the ledger commit never lists itself. No tag yet (first run, or a local
// dry run) → the entry covers just HEAD and the ledger starts there.
//
