...
// 1. a good check records what upstream offered, from the right URL, uncached
{
  ...
  const r = await checkOnce({ upstreamUrl: "https://example.invalid/releases/latest/download/", store, now: NOW, fetchFn, armed: true });
  const row = store.get();
  ...
  eq("goodRow", {
    ...
    v: row.upstream_version, ra: row.upstream_released_at, detail: row.detail, url: row.upstream_url, armed: row.armed,
  }, { at: NOW, ok: 1, v: MANIFEST.version, ra: MANIFEST.released_at, detail: "", url: "https://example.invalid/releases/latest/download/", armed: true });
...
{
  const state = {
    status: { upstream_version: "9a · x", upstream_released_at: "2026-08-19T00:00:00Z", upstream_url: "https://example.invalid/releases/latest/download/", armed: 1, last_check_at: 123, last_check_ok: 1, detail: "", last_install_at: 456, last_install_version: "9a · x", last_install_result: "ok", last_install_detail: "", updater_version: 1 },
    policy: { mode: "notify", soak_days: 3, install_now_version: "", install_now_at: 0 },
  };
  const d = await readPanel({ DB: panelDb(state) }, "old · x");
  out.panelRead = d.running === "old · x" && d.upstream.version === "9a · x" && d.upstream.url === "https://example.invalid/releases/latest/download/" && d.armed === true && d.lastCheck.ok === true &&
