...
// release.yml, candidate.yml, and the three hand-run ops workflows.
//
...
// run strictly after a green gate (deploy). release.yml is the same shape
// minus every Cloudflare credential. candidate.yml lives by the test job's
// rules. The ops workflows hold ADMIN_TOKEN against the live site, so they
// stay hand-dispatched, read-only, and inside the production environment.
...
console.log("✓ deploy.yml + release.yml + candidate.yml + ops workflow permission policy");
